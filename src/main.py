...
@cli.command()
@click.option("--auto", is_flag=True, help="Automation mode (no dialogs)")
@click.pass_context
def run(ctx: click.Context, auto: bool) -> None:
    """Start the full TUI workflow (discovery → generation → chat)."""
    config_path = ctx.obj["config_path"]
    config = load_config(config_path)

    # Check for AUTOMATION_MODE env var
    import os
    if os.environ.get("AUTOMATION_MODE", "").lower() in ("1", "true", "yes"):
        auto = True
