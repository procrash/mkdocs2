...
class OrchestrationEngine:
    ...
    def _get_active_analyst_ids(self, analyst_ids: list[str]) -> list[str]:
        """Filter out disabled models."""
        active = [m for m in analyst_ids if m not in self._disabled_models]
        if not active and analyst_ids:
    ...
    def task_id(task: GenerationTask) -> str:
        """Generate a stable ID for a task (for resume tracking)."""
        return f"{task.stakeholder}:{task.doc_type}:{task.file.relative_path}:{task.chunk_index}"
    ...
    async def run(self) -> PipelineResult:
        ...
        async def process_task(task: GenerationTask) -> GenerationResult:
            # Skip tasks already completed in a previous session
            tid = self.task_id(task)
            if tid in self._completed_task_ids:
                logger.info("Skipping already completed task: %s", tid)
                return GenerationResult(task=task, success=True, content="[resumed - already completed]")

            active_analysts = self._get_active_analyst_ids(analyst_ids)
            async with semaphore:
                return await self._process_single_task(
                    task, active_analysts, judge_id,
                )
